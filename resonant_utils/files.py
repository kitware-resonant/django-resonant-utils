...
@contextmanager
def field_file_to_local_path(field_file: FieldFile) -> Generator[Path]:
    """
    ...
    with field_file.open("rb"):
        ...
        if type(file_obj) is File:
            ...
            if file_obj.name is None:
                raise TypeError('The File object "field_file.file" does not have a "name".')
            ...
            if field_file.name is None:
                raise TypeError('The FieldFile object "field_file" does not have a "name".')
